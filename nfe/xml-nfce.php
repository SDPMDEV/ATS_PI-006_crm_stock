...
$query_customer_sql = "SELECT * FROM sma_companies WHERE id = '$clienteid'";
$query_customer = mysqli_query($connection, $query_customer_sql);
$cliente = mysqli_fetch_array($query_customer);  
  
$query_psale_sql = "SELECT * FROM sma_payments WHERE sale_id = '$pedido'";
$query_psale = mysqli_query($connection, $query_psale_sql);
...
$z = 0;
$query_bling_sql = "SELECT * FROM sma_sales WHERE id = '$pedido'";
$query_bling = mysqli_query($connection, $query_bling_sql);

while($bling = mysqli_fetch_array($query_bling)){

$prdbling_sql = "SELECT * FROM sma_sale_items WHERE sale_id = '".$bling['id']."'";
$prdbling = mysqli_query($connection, $prdbling_sql);
$produtobling = mysqli_fetch_array($prdbling);

$prdsbb_sql = "SELECT * FROM sma_products WHERE id = '".$produtobling['product_id']."'";
$prdsbb = mysqli_query($connection, $prdsbb_sql);
...
?>
<script>
alert('NFCe Gerado - Ambiente Teste Sem Certificado Digital');
document.location.href = ("libs/DanfeNFCe.php?nfe=nfce<?php echo $chave; ?>.xml");
</script>
